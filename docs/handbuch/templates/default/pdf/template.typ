...
#let setup-document(
    ...
    header: context {
      ...
      if show-header and not _is-special-page(cur-page) {
        ...
        grid(
          ...
          // Grundschutz++ Explorer: kleines Logo links vor Titel und Untertitel
          grid(
            columns: (auto, 1fr),
            column-gutter: 7pt,
            align: (left + horizon, left + top),
            image(logo-file, height: size-header-logo),
            [
  ...
  if show-cover {
    ...
    set par(justify: false)
    set text(hyphenate: false)
    // Grundschutz++ Explorer: Logo über dem Titel
    v(0.5cm)
    image(logo-file, width: size-cover-logo)
    v(0.8cm)
